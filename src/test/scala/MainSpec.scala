...
class MainSpec extends FunSpec {
	...
	describe("sanity tests"){
		it("returns an empty list of options when providing less or more then six items in the string"){
			assertResult(List()){ Scron._genOptions("1 1 1 1 1") }
			assertResult(List()){ Scron._genOptions("1 1 1 1 1 1 1") }
		}
	}

	describe("expected options generation"){
		it("allwos only numbers"){
			assertResult(List(List(1),List(1),List(1),List(1),List(1),List(1))){ Scron._genOptions("1 1 1 1 1 1") } }
		it("allows astisks"){ 
			assertResult(Scron.maxPool) { Scron._genOptions("* * * * * *") } }
